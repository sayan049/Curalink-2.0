...
Return ONLY this JSON:
{
  ...
  buildUserPrompt(
    ...
    const mentionedMutations = MUTATION_PATTERNS.filter((m) =>
      queryLower.includes(m),
    );

    const getFailedTreatment = () => {
      if (mentionedDrugs.length > 0) return mentionedDrugs[0];
      if (queryLower.includes("immunotherapy") || queryLower.includes("ici"))
        return "immunotherapy";
      if (queryLower.includes("egfr") || queryLower.includes("tki"))
        return "EGFR-TKI";
    ...
    if (isSupplementQuery) {
      ...
      clinicalAnswerNeeded = `Mechanism of action / biological pathway per the papers provided.`;
  ...
  _extractSafetyHint(abstract, title = "") {
    ...
    if (titleL.includes("without increased pneumonia risk"))
      return "Comparative safety: no increased risk of pneumonia";
    if (titleL.includes("without increased risk"))
      return "Comparative safety: no increased risk of adverse events";
    if (titleL.includes("no increased risk"))
      return "Comparative safety: no increased risk of adverse events";
    ...
    if (negMatch)
      return `Comparative safety: no increased risk of ${negMatch[1].toLowerCase()}`;

    if (
      abstractL.includes("without increased") ||
      abstractL.includes("no increased risk") ||
      abstractL.includes("no significant increase") ||
      abstractL.includes("did not increase") ||
      abstractL.includes("not associated with increased")
    ) {
      const m =
        abstract.match(
          /without\s+increased\s+(?:risk\s+of\s+)?(\w+(?:\s+\w+){0,2})/i,
        ) ||
        abstract.match(
          /no\s+(?:significant\s+)?increased?\s+(?:risk\s+of\s+)?(\w+(?:\s+\w+){0,2})/i,
        );
      return m
    ...
    const patterns = [
      {
        regex:
          /grade\s*[3≥]\s*(?:or\s*(?:higher|above|4)|\/4)?\s*(?:adverse\s*events?)?\s*(?:occurred|reported|in|were)\s*(\d+\.?\d*\s*%)/i,
        label: "Grade 3+ AE",
      },
      {
        regex:
          /(\d+\.?\d*\s*%)\s*(?:of\s*patients?\s*)?(?:experienced|had|reported)\s*(?:any\s*)?grade\s*(?:3|≥3|4)/i,
        label: "Grade 3+ AE",
      },
      {
        regex:
          /treatment[\s-]related adverse events?\s*(?:occurred|reported|in)\s+(\d+\.?\d*\s*%)/i,
        label: "Treatment-related AE",
      },
      {
        regex:
          /(\d+\.?\d*\s*%)\s*(?:of patients?\s*)?(?:had|experienced)\s*treatment[\s-]related/i,
        label: "Treatment-related AE",
      },
      {
        regex:
      ...
      {
        regex:
          /immune[\s-]related adverse events?\s*(?:occurred|in|rate).*?(\d+\.?\d*\s*%)/i,
        label: "irAE rate",
      },
      {
        regex:
          /(\d+\.?\d*\s*%)\s*(?:of patients?\s*)?(?:had|experienced)\s*immune[\s-]related/i,
        label: "irAE rate",
      },
      {
        regex:
          /discontinu(?:ed|ation)\s*due\s+to\s+(?:adverse|toxicity|treatment-related)[^.]*?(\d+\.?\d*\s*%)/i,
        label: "Discontinuation rate",
      },
      {
        regex:
          /discontinu(?:ed|ation)\s*rate\s*(?:was|of|:|\s*=)\s*(\d+\.?\d*\s*%)/i,
        label: "Discontinuation rate",
      },
      {
        regex:
          /(\d+\.?\d*\s*%)\s*(?:of patients?\s*)?discontinued\s*(?:due\s+to|because\s+of)\s*(?:adverse|toxicity)/i,
        label: "Discontinuation rate",
      },
      {
        regex:
      ...
      {
        regex:
          /(well[\s-]tolerated|manageable safety profile|no new safety signals|acceptable safety|favorable safety)/i,
  ...
  _classifyEvidenceTier(title, abstract) {
    ...
    if (
      combined.includes("mendelian randomisation") ||
      combined.includes("mendelian randomization") ||
      combined.includes("case-cohort") ||
      combined.includes("case cohort")
    )
  ...
  // FIX A: Fully expanded IGNORE pattern
  // Blocks ALL non-result sentence types:
  //   - Database/registry/search language
  //   - Methods process language
  //   - Data extraction methodology ("single extraction", "were used to")
  //   - Background / epidemiological framing ("leading cause", "is one of")
  //   - Scope description sentences ("this review", "we searched")
  //   - Purpose/aim sentences ("to review", "to assess", "the aim of")
  //   - Hypothesis / future tense ("may play", "is not yet")
  //   - Multi-cancer framing ("cardiovascular disease and cancer")
  //
  // Works for ALL query types — treatment, supplement, diagnosis,
  // prognosis, mechanism, comparison, researchers, clinical trials.
  // ════════════════════════════════════════════════════════════════════════
  _extractOutcomeHint(abstract) {
    ...
    const patterns = [
      ...
      {
        regex: /progression[\s-]free survival.*?(\d+\.?\d*\s*months?)/i,
        label: "PFS",
      },
      ...
      {
        regex: /sensitivity\s*(?:of|was|:)?\s*(\d+\.?\d*\s*%)/i,
        label: "Sensitivity",
      },
      {
        regex: /specificity\s*(?:of|was|:)?\s*(\d+\.?\d*\s*%)/i,
        label: "Specificity",
      },
      {
    ...
    // ── FULLY EXPANDED IGNORE — blocks all non-result sentence types ───────
    // Applies universally to treatment, supplement, diagnosis, mechanism,
    // comparison, prognosis, researchers, and clinical trial queries.
    const IGNORE = new RegExp(
      [
        ...
        "will be",
        "will recruit",
        "investigators?\\s+independently",
        "investigators?\\s+screened",
        "data\\s+extraction",
        "quality\\s+assessment",
        "risk\\s+of\\s+bias",
        "two\\s+(?:authors?|reviewers?|investigators?)\\s+independently",
        "independently\\s+(?:screened|reviewed|extracted|assessed)",

        // ── FIX A: Data extraction methodology sentences ──
        // "Single extraction, verified by a second reviewer" → blocked
        // "The pooled relative risks were used to assess" → blocked
        "single\\s+extraction",
        "double\\s+extraction",
        "independently\\s+extracted",
        "verified\\s+by\\s+(?:a\\s+second|another)\\s+reviewer",
        "pooled\\s+relative\\s+risks?\\s+(?:\\(rr\\s*\\))?\\s+with\\s+95",
        "were\\s+used\\s+to\\s+assess",
        "were\\s+used\\s+to\\s+evaluate",
        "were\\s+used\\s+to\\s+calculate",
        "were\\s+used\\s+to\\s+estimate",
        "were\\s+used\\s+to\\s+determine",
        "was\\s+used\\s+to\\s+assess",
        "was\\s+used\\s+to\\s+evaluate",
        "was\\s+used\\s+to\\s+calculate",
        "was\\s+performed",
        "were\\s+performed",
        "was\\s+conducted",
        "were\\s+conducted",
        ...
        "were\\s+enrolled",
        "were\\s+eligible",
        "meta-analyses\\s+focusing",
        "studies\\s+focusing",
        "reviews\\s+focusing",
        "articles\\s+focusing",
        "trials\\s+focusing",

        // ── Study scope / process sentences ──
        "we\\s+searched",
        "we\\s+conducted",
        "we\\s+performed",
        "we\\s+reviewed",
        ...
        "this\\s+review",
        "this\\s+study\\s+aims",
        "this\\s+meta-analysis",
        "this\\s+systematic",
        "this\\s+article",

        // ── Purpose / aim sentences ──
        // "To review the benefits and harms of vitamin..." → blocked
        "to\\s+review\\s+the",
        "to\\s+assess\\s+the",
        "to\\s+evaluate\\s+the",
        "to\\s+investigate\\s+the",
        "to\\s+determine\\s+the",
        "to\\s+examine\\s+the",
        "to\\s+describe\\s+the",
        "to\\s+summarize",
        "to\\s+summarise",
        "the\\s+aim\\s+of",
        "the\\s+purpose\\s+of",
        "the\\s+objective\\s+of",
        "the\\s+goal\\s+of",
        "we\\s+aimed\\s+to",
        "we\\s+sought\\s+to",

        // ── Background / epidemiological framing ──
        "are\\s+the\\s+\\d+",
        "is\\s+the\\s+\\d+",
        "leading\\s+cause",
        "leading\\s+causes",
        "has\\s+been\\s+proposed",
        "have\\s+been\\s+proposed",
        "is\\s+a\\s+major",
        "is\\s+an\\s+important",
        "remains\\s+a",
        "continues\\s+to\\s+be",
    ...
    // A result sentence must:
    // 1. Be 40-200 chars (not a fragment, not a paragraph)
    // 2. Not match IGNORE
    // 3. Contain at least one number
    // 4. Contain at least one result-language word
    const resultSentence = sentences.find((s) => {
      if (s.length < 40 || s.length > 200) return false;
      if (IGNORE.test(s)) return false;
      if (!/\d/.test(s)) return false;
      if (
        !/(response|survival|hazard|risk|reduction|improved|benefit|associated|efficacy|p\s*[<=>]|significant|mortality|recurrence|incidence|outcome|progression)/i.test(
          s,
        )
      )
        return false;
    ...
    // Last resort — first non-ignored sentence of any kind
    const firstSentence = sentences.find(
      (s) => s.length > 30 && s.length < 200 && !IGNORE.test(s),
    );
  ...
  _extractApplicability(pub) {
    ...
    const isMechanistic =
      /\bin vitro\b|\bcell line\b|\bmouse model\b|\bmurine\b|\banimal model\b|\bxenograft\b/i.test(
        `${title} ${firstSentence}`,
      );
    if (isMechanistic)
      return "preclinical / in vitro study (results may not directly apply to patients)";
    ...
    if (!hasNSCLC && !hasSCLC) {
      ...
      else if (/\bsmall[-\s]cell\b|\bsclc\b/i.test(firstSentence))
        tags.push("SCLC");
    }

    if (/squamous\s+(?:cell\s+)?(?:carcinoma|nsclc|non-small)/i.test(title))
      tags.push("squamous subtype");
    ...
    const negated = (marker) =>
      new RegExp(
        ...
        "i",
      ).test(`${title} ${firstSentence}`);

    if (
      /\balk[-\s]positive\b|\balk\s+(?:inhibitor|rearrangement|fusion|mutant|amplification)\b/i.test(
        title,
      )
    )
    ...
    if (/egfr\s+exon\s+20|exon\s+20\s+insert/i.test(title))
      ...
      /\begfr[-\s](?:mutant|mutation|positive|activating|sensitizing)\b|\begfr\s+tki\b/i.test(
        title,
      ) &&
      !negated("egfr")
    )
    ...
    if (
      /\bpd-l1\b/i.test(title) &&
      !negated("pd-l1") &&
      !/regardless of pd-l1|unselected|all\s*comers/i.test(
        `${title} ${firstSentence}`,
      )
    )
    ...
    if (/\bros1[-\s]positive\b|\bros1\s+fusion\b/i.test(title))
      tags.push("ROS1-positive");
    if (/\bret[-\s]positive\b|\bret\s+fusion\b/i.test(title))
      tags.push("RET-positive");
    if (/\bher2[-\s](?:positive|mutant|overexpress)\b/i.test(title))
      tags.push("HER2-positive");
    ...
    if (/brca\s*[12]\s+mutant|\bbrca[-\s]mutant\b/i.test(title))
      tags.push("BRCA-mutant");
    ...
    if (/\badvanced\b|\bmetastatic\b/i.test(title))
      tags.push("advanced/metastatic");
    if (/\boligometastatic\b|\boligo[-\s]metastatic\b/i.test(title))
      tags.push("oligometastatic");
    if (/\bneoadjuvant\b/i.test(title)) tags.push("neoadjuvant setting");
    if (/\badjuvant\b/i.test(title) && !/\bneoadjuvant\b/i.test(title))
      tags.push("adjuvant setting");
    if (/\bperioperative\b/i.test(title)) tags.push("perioperative setting");
    if (/\bsurgical\b|\bpost-surgical\b|\bpost-resection\b/i.test(title))
      tags.push("post-surgical");

    if (/\bfirst[-\s]line\b/i.test(title)) tags.push("first-line");
    if (/\bsecond[-\s]line\b/i.test(title)) tags.push("second-line");
    if (/\bthird[-\s]line\b|\blater[-\s]line\b/i.test(title))
      tags.push("later-line");
    if (
      /post[-\s]tki|after\s+tki|tki[-\s](?:resistant|refractory|failure)/i.test(
        title,
      )
    )
      tags.push("post-TKI");
    if (
      /after\s+(?:anti-)?pd-1|post[-\s]immunotherapy|immunotherapy[-\s](?:refractory|failure)/i.test(
        title,
      )
    )
      tags.push("post-immunotherapy");
    if (/\bsalvage\b|\brefractory\b/i.test(title))
      tags.push("refractory/salvage setting");

    if (/\belderly\b|\bolder\s+patients\b/i.test(title))
      tags.push("elderly patients");
    if (/\bpediatric\b|\bchildren\b/i.test(title))
      tags.push("pediatric patients");

    if (/\bheart\s+failure\b|\bhfref\b|\bhfpef\b/i.test(title))
      tags.push("heart failure");
    if (/\batrial\s+fibrillation\b|\bafib\b/i.test(title))
      tags.push("atrial fibrillation");
    if (/\bcoronary\s+artery\s+disease\b|\bcad\b/i.test(title))
      tags.push("coronary artery disease");
    if (/\bmyocardial\s+infarction\b/i.test(title))
      tags.push("myocardial infarction");
    if (/\bhypertension\b/i.test(title)) tags.push("hypertension");
    if (/\bstroke\b|\bischemic\s+stroke\b/i.test(title)) tags.push("stroke");

    if (/\btype\s+2\s+diabetes\b|\bt2dm\b/i.test(title))
      tags.push("type 2 diabetes");
    if (/\btype\s+1\s+diabetes\b|\bt1dm\b/i.test(title))
      tags.push("type 1 diabetes");
    if (/\bobesity\b/i.test(title)) tags.push("obesity");
    if (/\bmetabolic\s+syndrome\b/i.test(title))
      tags.push("metabolic syndrome");

    if (/\bparkinson'?s?\s+disease\b/i.test(title))
      tags.push("Parkinson's disease");
    if (/\balzheimer'?s?\s+disease\b/i.test(title))
      tags.push("Alzheimer's disease");
    if (/\bmultiple\s+sclerosis\b/i.test(title))
      tags.push("multiple sclerosis");
    if (/\bepilep/i.test(title)) tags.push("epilepsy");
    if (/\bdeep\s+brain\s+stimulation\b|\bdbs\b/i.test(title))
      tags.push("DBS candidates");

    if (/\bchronic\s+obstructive\s+pulmonary\b|\bcopd\b/i.test(title))
      tags.push("COPD");
    if (/\basthma\b/i.test(title)) tags.push("asthma");
    if (/\bidiopathic\s+pulmonary\s+fibrosis\b|\bipf\b/i.test(title))
      tags.push("IPF");

    if (/\bchronic\s+kidney\s+disease\b|\bckd\b/i.test(title))
      tags.push("chronic kidney disease");
    if (/\bdialysis\b/i.test(title)) tags.push("dialysis patients");

    if (/\brheumatoid\s+arthritis\b/i.test(title))
      tags.push("rheumatoid arthritis");
    if (/\bsystemic\s+lupus\b|\bsle\b/i.test(title)) tags.push("SLE/lupus");
    if (/\bpsoriasis\b/i.test(title)) tags.push("psoriasis");
    if (/\bcrohn'?s\b|\binflammatory\s+bowel\b/i.test(title))
      tags.push("inflammatory bowel disease");

    if (/\bmajor\s+depressive\b|\bdepression\b/i.test(title))
    ...
    if (/\btuberculosis\b/i.test(title)) tags.push("tuberculosis");
    if (/\bhepatitis\s+[bc]\b/i.test(title)) tags.push("viral hepatitis");

    if (!/nsclc|sclc|non-small\s*cell|small[-\s]cell|lung/i.test(title)) {
      if (/\bbreast\s+cancer\b/i.test(title)) tags.push("breast cancer");
      if (/\bcolorectal\s+cancer\b/i.test(title))
        tags.push("colorectal cancer");
      if (/\bprostate\s+cancer\b/i.test(title)) tags.push("prostate cancer");
      if (/\bpancreatic\s+cancer\b/i.test(title))
        tags.push("pancreatic cancer");
      if (/\bgastric\s+cancer\b/i.test(title)) tags.push("gastric cancer");
      if (/\bovarian\s+cancer\b/i.test(title)) tags.push("ovarian cancer");
      if (/\bhepatocellular\b/i.test(title))
        tags.push("hepatocellular carcinoma");
      ...
      if (/\bglioblastoma\b|\bglioma\b/i.test(title)) tags.push("brain tumor");
    ...
    if (/\bvitamin\s+[abcek]\b/i.test(title))
      tags.push("vitamin supplementation");
    if (/\bomega[-\s]3\b|\bfish\s+oil\b/i.test(title))
      tags.push("omega-3 supplementation");
    if (/\bzinc\b/i.test(title)) tags.push("zinc supplementation");
    if (/\bmagnesium\b/i.test(title)) tags.push("magnesium supplementation");
    if (/\bcurcumin\b|\bturmeric\b/i.test(title))
      tags.push("curcumin/turmeric");
  ...
  _inferIntentFromQuery(query = "") {
    ...
    if (/side effect|adverse effect|toxicity|tolerability/i.test(q))
      return "side_effects";
    if (
      /vitamin|supplement|mineral|herb|can i take|should i take|is it safe to|diet|food|eat|drink/i.test(
        q,
      )
    )
      return "safety_efficacy";
    if (
      / vs | versus |compare|which is better|head-to-head|difference between/i.test(
        q,
      )
    )
      return "comparison";
    if (
      /prognosis|survival rate|life expectancy|mortality|5.year survival|outlook/i.test(
        q,
      )
    )
      return "prognosis";
    if (/symptom|diagnos|screening|biomarker|detect/i.test(q))
      return "symptoms_diagnosis";
    if (/mechanism|how does|pathway|why does|how it works/i.test(q))
      return "mechanism";
    if (/prevent|reduce risk|lower risk|prophylaxis|preventive/i.test(q))
      return "prevention";
    if (/clinical trial|trial for|trials in|enrolling|recruiting/i.test(q))
      return "clinical_trials";
    if (/researcher|expert|scientist|pioneer|who studies/i.test(q))
      return "researchers";
    if (
      /recent|latest research|new findings|current research|emerging/i.test(q)
    )
      return "recent_research";
    if (
      /treatment|therapy|medication|drug for|how to treat|immunotherapy|chemotherapy/i.test(
        q,
      )
    )
      return "treatment_solutions";
    return "general";
  }

  // ════════════════════════════════════════════════════════════════════════
  // Research insights — deterministic synthesis
  // ════════════════════════════════════════════════════════════════════════
  _buildResearchInsights(publications = [], query = "") {
    ...
    const evidenceTypes = [
      ...new Set(
        publications.map(
          (p) =>
            this._classifyEvidenceTier(p.title || "", p.abstract || "").split(
              " —",
            )[0],
        ),
      ),
    ...
    const applicabilities = publications.map((p) =>
      this._extractApplicability(p),
    );
    const narrowPapers = applicabilities.filter(
      (a) =>
        !/broad disease population|preclinical/i.test(a) &&
        a !== "broad disease population",
    ).length;
    ...
    const allText = publications
      ...
      .toLowerCase();
    const topThemes = [];
    if (/immune checkpoint|pd-1|pd-l1|immunotherapy/i.test(allText))
      topThemes.push("immune checkpoint inhibitor strategies");
    if (/egfr|alk|ros1|ret|kras|targeted therapy/i.test(allText))
      topThemes.push("biomarker-directed targeted therapies");
    if (/radiation|radiotherapy|sabr|ablation/i.test(allText))
      topThemes.push("combined local therapy approaches");
    if (/supplement|vitamin|dietary|lifestyle/i.test(allText))
      topThemes.push("lifestyle and supplementation evidence");
    if (/hba1c|glycemic|sglt2|glp-1/i.test(allText))
      topThemes.push("glycemic control strategies");
    if (/dopamine|levodopa|motor symptoms/i.test(allText))
      topThemes.push("motor symptom management");
    if (/amyloid|tau|cognitive|dementia/i.test(allText))
      topThemes.push("cognitive and neurodegeneration therapies");
    if (/cardiovascular|mace|blood pressure|statin/i.test(allText))
      topThemes.push("cardiovascular risk reduction");
    if (/diagnosis|sensitivity|specificity|biomarker/i.test(allText))
      topThemes.push("diagnostic and biomarker evidence");
    if (/mechanism|pathway|signaling|molecular/i.test(allText))
      topThemes.push("mechanistic and molecular insights");

    let insight = `Evidence strength includes ${evidenceTypes.join(", ")}. `;
    if (topThemes.length > 0)
      insight += `Key treatment themes identified: ${topThemes.slice(0, 3).join(", ")}. `;
    if (narrowPapers > 0) {
  ...
  // ════════════════════════════════════════════════════════════════════════
  // Recommendations — all query types, evidence-strength sorted
  // ════════════════════════════════════════════════════════════════════════
  _buildRecommendations(publications, query = "", intentType = "general") {
    ...
    const classified = publications.map((pub) => {
      ...
      else if (lower.includes("meta-analysis") || lower.includes("systematic"))
        strength = 4;
      else if (lower.includes("phase 2")) strength = 3;
      else if (lower.includes("prospective") || lower.includes("real-world"))
        strength = 2;
      else if (
    ...
    if (selected.length < 3) {
      for (const item of sorted) {
        if (selected.length >= 3) break;
        if (!selected.find((s) => s.pub === item.pub)) selected.push(item);
      }
    }
    ...
    return selected.map(({ pub, tierShort, outcome, applicability }) => {
      ...
      const isSubgroup =
        applicability && applicability !== "broad disease population";
      ...
      switch (intentType) {
        ...
        case "mechanism":
          return hasData
            ...
            : `${tierShort} mechanistic evidence from "${title}..."${scopeStr}. Discuss the clinical relevance with your specialist.`;
        case "clinical_trials":
          return isSubgroup
            ? `For ${applicability}: "${title}..." (${tierShort}) may be relevant. Check eligibility on ClinicalTrials.gov or with your physician.`
            : `"${title}..." (${tierShort}) may be relevant. Check eligibility on ClinicalTrials.gov or discuss with your physician.`;
        case "researchers":
          return hasData
            ? `${tierShort} work${scopeStr}: "${title}..." reported ${outcome}. This represents a key research contribution.`
        ...
        default:
          ...
          return isSubgroup
            ? `Evidence from "${title}..." (${tierShort}) applies mainly to ${applicability}. Discuss with your specialist.`
            : `${tierShort} evidence from "${title}..." — consult your specialist for relevance.`;
      }
    });
  }
  ...
  _verifyGrounding(keyFindings = [], publications = []) {
    return keyFindings.map((finding) => {
      ...
      if (paperIdx < 0 || paperIdx >= publications.length) {
        console.warn(`⚠️  Citation [${paperIdx + 1}] out of range`);
      ...
      const numbersInFinding =
        finding.match(/\b\d+(\.\d+)?(%| months?| mg| kg| mmhg)?\b/gi) || [];
      ...
      if (hallucinations.length > 0) {
        console.warn(
          `⚠️  Grounding failure [${paperIdx + 1}]:`,
          hallucinations,
        );
        let cleaned = finding;
        hallucinations.forEach((h) => {
          cleaned = cleaned.replace(h, "[unverified]");
        });
        return cleaned;
      }
      return finding;
  ...
  async generateMedicalResponse(
    ...
    let userPrompt = this.buildUserPrompt(
      ...
      {
        abstractChars,
        includeUrls: budget.includeUrls,
        maxPapers: budget.maxPapers,
    ...
    let estimatedInput =
      ...
      this._estimateTokens(
        workingHistory.map((m) => m.content || "").join(" "),
      );

    let shrinkPasses = 0;
    while (
      estimatedInput + budget.maxOutputTokens > budget.maxInputTokens &&
      shrinkPasses < 4
    ) {
      shrinkPasses++;
      if (workingPubs.length > 3) {
        ...
        console.log(
          `   ⚠️  Token budget: reduced papers to ${workingPubs.length}`,
        );
      } else if (abstractChars > 150) {
        abstractChars = Math.max(150, abstractChars - 60);
        console.log(
          `   ⚠️  Token budget: reduced abstract to ${abstractChars} chars`,
        );
      } else {
        break;
      }
    ...
    const safeMaxTokens = Math.max(
      ...
      Math.min(
        ...
        budget.maxInputTokens - estimatedInput - 100,
      ),
    );
    ...
    console.log(
      `   Publications: ${workingPubs.length} (of ${publications.length})`,
    );
    ...
    if (context._clinicalFocus)
      console.log(`   Focus:        "${context._clinicalFocus}"`);
  ...
  parseStructuredResponse(
    ...
    const intentType =
      context?._intent?.type || this._inferIntentFromQuery(query);
    ...
    try {
      ...
      if (Array.isArray(parsed.keyFindings)) {
        parsed.keyFindings = this._verifyGrounding(
          parsed.keyFindings,
          publications,
        );
      }
      ...
      if (Array.isArray(parsed.keyFindings)) {
        ...
        parsed.keyFindings = parsed.keyFindings.filter((finding) => {
          ...
          if (cited.has(primary)) return false;
          cited.add(primary);
          return true;
        });
        console.log(
          `   📝 keyFindings: ${parsed.keyFindings.length} (after dedup + grounding)`,
        );
        ...
        if (parsed.keyFindings.length === 0 && publications.length > 0) {
          ...
          parsed.keyFindings = publications.slice(0, 6).map((pub, idx) => {
            ...
            const hasData = outcome !== "See abstract below";
            const scope =
              applicability !== "broad disease population"
                ? ` (${applicability})`
                : "";
            return hasData
      ...
      if (
        ...
        (parsed.conditionOverview.includes("leading cause") &&
          parsed.conditionOverview.length < 150)
      ) {
      ...
      if (
        ...
        parsed.researchInsights = this._buildResearchInsights(
          publications,
          query,
        );
      }
      ...
      if (
        ...
        parsed.recommendations = this._buildRecommendations(
          publications,
          query,
          intentType,
        );
      ...
      if (
        ...
        if (clinicalTrials.length > 0)
          parsed.clinicalTrialsSummary =
            this._buildTrialSummary(clinicalTrials);
      }
  ...
  _buildSafetyFromHints(publications = []) {
    ...
    publications.slice(0, 8).forEach((pub, idx) => {
      ...
      if (
        titleL.includes("without increased risk") ||
        titleL.includes("no increased risk")
      ) {
        const risk = titleL.includes("pneumonia")
          ? "pneumonia"
          : titleL.includes("bleeding")
            ? "bleeding"
            : titleL.includes("toxicity")
              ? "toxicity"
              : "adverse events";
        safetyFindings.push(
          `${paperNum} reported no increased risk of ${risk}.`,
        );
        return;
      }
      ...
      if (titleL.includes("well-tolerated") || titleL.includes("manageable")) {
        safetyFindings.push(
          `${paperNum} described a well-tolerated safety profile.`,
        );
        return;
      }

      if (
        tier.includes("Observational") ||
        tier.includes("Meta-analysis") ||
        tier.includes("Retrospective") ||
        tier.includes("Case-cohort") ||
        tier.includes("Mendelian")
      ) {
        safetyFindings.push(
      ...
      if (
        titleL.includes("safety and efficacy") ||
        titleL.includes("efficacy and safety")
      ) {
        const broadMatch =
          (pub.abstract || "").match(
  ...
  // ════════════════════════════════════════════════════════════════════════
  // _buildConditionOverview — uses true latest year across all papers
  // ════════════════════════════════════════════════════════════════════════
  _buildConditionOverview(query, publications) {
    ...
    const years = publications.map((p) => p.year).filter((y) => y && y > 1900);
    const latestYear =
    ...
    overview += `These findings represent the available evidence base as of ${latestYear}.`;
    return overview;
  }
  ...
  buildFallbackStructuredResponse(
    ...
    return {
      ...
      keyFindings:
        findings.length > 0
          ? findings
          : ["No specific findings extracted. Please refine your query."],
      ...
      recommendations: this._buildRecommendations(
        publications,
        query,
        intentType,
      ),
