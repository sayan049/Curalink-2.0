  ...
  async search(query, perPage = 200, page = 1) {
    try {
      ...
      const publications = works.map((work) => ({
        ...
        // ✅ NEW: OpenAlex semantic concepts
        // Each concept has a name and a relevance score (0-1)
        // Used in rankingService for intent validation
        // Helps identify predictor papers vs treatment papers
        concepts: (work.concepts || [])
          .filter((c) => c.score > 0.3)
          .map((c) => ({
            name: (c.display_name || "").toLowerCase(),
            score: c.score || 0,
          }))
          .slice(0, 15),
      }));
